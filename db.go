...
func (conn *DBConn) DeleteAll() error {
	...
	if _, err := conn.db.Exec("DELETE FROM IOCList"); err != nil {
		return err
	}
	return nil
}
...
// SaveHost inserts new host into IOCList table
func (conn *DBConn) SaveHost(providerGuid string, host *Host) error {
	query := "INSERT INTO IOCList(ProviderGuid, Name, FirstSeen, Active) VALUES($1, $2, $3, $4)"
...
func (conn *DBConn) GetAllHosts() ([]*Host, error) {
	hosts := make([]*Host, 0)
	query := "SELECT ProviderGuid,Name,FirstSeen,Active FROM IOCList ORDER BY Id"
